...
def main():
    ...
    for i in ids:
        ...
        with open(labels[i].replace(label_root, result_root),'w') as new_f:
            pred_labels = ''
            for class_, truncated, occluded, delta, alpha, theta, box2d, bias, gt in zip(CLASSes, TRUNCATEDs, OCCLUDEDs, delta_DIMs, Alphas, THETAs, box2ds, depth_BIAs, depth_GT):
                ...
                calc_depth = loc[2]
                depth_width = calc_depth_with_alpha_theta(img_W, box2d, cam_to_img, dim[1], dim[2], alpha, truncated)
