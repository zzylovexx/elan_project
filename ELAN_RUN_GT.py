...
def main():
    ...
    # dim averages
    averages_all = ClassAverages([], 'ELAN_class_averages.txt')
    cam_to_img = np.array([
    ...
    for i in range(len(images)):
        ...
        with open(labels[i].replace(label_root, result_root),'w') as new_f:
            pred_labels = ''
            for class_, truncated, occluded, delta, alpha, theta, box_2d, gt in zip(CLASSes, TRUNCATEDs, OCCLUDEDs, delta_DIMs, Alphas, THETAs, BOX2Ds, depth_GT):
