...
def main():
    ...
    for epoch in range(1, epochs+1):
        ...
        for id_ in trainset:
            ...
            crops_flip = torch.stack(crops_flip).to(device)
            with torch.no_grad():
                [_, _, reg_dim_delta_flip] = model(crops_flip)
                flip_loss = F.l1_loss(reg_dim_delta, reg_dim_delta_flip, reduction='mean')
            loss += flip_loss
            ...
            if passes % 100 == 0:
                print("--- epoch %s | passes %s --- [loss: %.3f],[bin_loss:%.3f],[residual_loss:%.3f],[dim_loss:%.3f],[consist_loss:%.3f],[angle_loss:%.3f],[flip_loss:%.3f]" \
                        %(epoch, passes, loss.item(), bin_loss.item(), residual_loss.item(), dim_loss.item(), consist_loss.item(), angle_loss.item(), flip_loss.item()))
                if is_group and epoch > warm_up:
