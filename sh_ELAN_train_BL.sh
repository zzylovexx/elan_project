...
then
    PKL=$PKL"_dep"
    R_PATH1=$R_PATH1"_dep"
    R_PATH2=$R_PATH2"_dep"
fi
if [ $DEPTH = $TWO ]
then
    PKL=$PKL"_depA"
    R_PATH1=$R_PATH1"_depA"
    R_PATH2=$R_PATH2"_depA"
fi
if [ $IOU = $ONE ]
then
    PKL=$PKL"_iou"
    R_PATH1=$R_PATH1"_iou"
    R_PATH2=$R_PATH2"_iou"
fi
if [ $IOU = $TWO ]
then
    PKL=$PKL"_iouA"
    R_PATH1=$R_PATH1"_iouA"
    R_PATH2=$R_PATH2"_iouA"
fi
if [ $AUGMENT = $ONE ]
then
    PKL=$PKL"_aug"
